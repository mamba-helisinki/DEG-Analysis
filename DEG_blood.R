...
# Read Data
countData <- read.table("C:/Users/HP/Desktop/Dhaval/DDP/Results/merged_AD_healthy.txt", header=TRUE, row.names=1)

countData1 <- countData[, !(names(countData) %in% c("H1B","H3B","H4B","AD4","AD6","AD7"))]

# Create colData with the 'condition' variable
colData <- data.frame(condition = c("diseased","diseased","healthy", "healthy"))
...
column_order <- c("AD2","AD3","H6B","H8B")
...
custom_column_order <- c("AD2","AD3","H6B","H8B")  # Add your sample names in the desired order
...
# Define the column order for the heatmap
column_order <- c('H6B','H8B','AD2','AD3')
...
z_scores_all <- scale(merged_counts_DE_alll, center = TRUE, scale = TRUE)
...
common_genes_blood <- merge(merged_counts_genes_brain, gene_names_df, by = "gene_name.y", all.x = FALSE)
###########################################################################################
# Assuming countData is your data frame

# Select columns corresponding to Alzheimer's disease (AD) samples
ad_columns <- grep("^AD", colnames(countData1), value = TRUE)

# Select columns corresponding to healthy samples
healthy_columns <- grep("^H", colnames(countData1), value = TRUE)

# Calculate the row-wise average for Alzheimer's disease samples and healthy samples
ad_average <- rowMeans(countData1[, ad_columns])
healthy_average <- rowMeans(countData1[, healthy_columns])

# Create a data frame with the row-wise averages and appropriate column names
average_df <- data.frame(Average_Counts_AD = ad_average, Average_Counts_Healthy = healthy_average)

# Print or use the new data frame as needed
print(average_df)

# Calculate the log2 fold change between AD and healthy averages
average_df$log2FC <- log2(average_df$Average_Counts_AD / average_df$Average_Counts_Healthy)

# Subset the data frame to include only rows with finite log2FC values
average_df <- average_df[is.finite(average_df$log2FC), ]
...
topGenes_df <- merge(topGenes, unique_gene_info, by = "gene_id", all.x = TRUE)
